...
func fixtureBytes(t *testing.T, name string) []byte {
	t.Helper()
	data, err := os.ReadFile(filepath.Join("..", "..", "..", "testdata", "claudecode", name+".json"))
	if err != nil {
		t.Fatal(err)
	}
	return data
...
func TestParseFixtures(t *testing.T) {
	for _, scenario := range []string{"normal", "version_change", "missing_fields"} {
		t.Run(scenario, func(t *testing.T) {
			adapter := New()
			raw := adapters.RawSession{Source: adapters.SourceClaudeCode, RawJSON: fixtureBytes(t, scenario)}
			meta, events, err := adapter.Parse(raw)
			if err != nil {
				t.Fatal(err)
			}
...
func TestParseIsDeterministic(t *testing.T) {
	adapter := New()
	raw := adapters.RawSession{Source: adapters.SourceClaudeCode, RawJSON: fixtureBytes(t, "normal")}
...
func TestDetectVersion(t *testing.T) {
	adapter := New()
	version, err := adapter.DetectVersion(adapters.RawSession{Source: adapters.SourceClaudeCode, RawJSON: fixtureBytes(t, "normal")})
	if err != nil {
		t.Fatal(err)
	}
	if version.HarnessVersion != "2.14.0" || version.Model == "" {
		t.Fatalf("version = %#v", version)
	}
	if _, err := adapter.DetectVersion(adapters.RawSession{Source: adapters.SourceClaudeCode, RawJSON: []byte("{")}); err == nil {
...
func TestParsePreservesExplicitOutcomeEvidence(t *testing.T) {
	raw := []byte(`{"session":{"id":"outcome","started_at":"2026-08-20T09:00:00Z","ended_at":"2026-08-20T09:03:00Z"},"messages":[{"id":"m1","timestamp":"2026-08-20T09:01:00Z","asset_invocations":[{"asset_id":"skill:fixture","followed":true,"violated":true}]}]}`)
	_, events, err := New().Parse(adapters.RawSession{Source: adapters.SourceClaudeCode, RawJSON: raw})
	if err != nil {
		t.Fatal(err)
	}
...
func TestParsePreservesExplicitToolError(t *testing.T) {
	raw := []byte("{\"session\":{\"id\":\"tool-error\",\"started_at\":\"2026-08-20T09:00:00Z\",\"ended_at\":\"2026-08-20T09:03:00Z\"},\"messages\":[{\"id\":\"result-1\",\"timestamp\":\"2026-08-20T09:01:00Z\",\"role\":\"tool\",\"kind\":\"tool_result\",\"tool_output\":\"permission denied\",\"is_error\":true}]}")
	_, events, err := New().Parse(adapters.RawSession{Source: adapters.SourceClaudeCode, RawJSON: raw})
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Fatalf("tool error evidence missing: %#v", events)
}
