...
func TestIngestSessionFillsMissingMetadataWithoutOverwritingFacts(t *testing.T) {
	...
	if started != testTime(1).Format("2006-01-02T15:04:05Z07:00") || ended != testTime(2).Format("2006-01-02T15:04:05Z07:00") || harness != "2.14" || model != "synthetic-model" || cwd != "/synthetic/project" || title != "真实会话标题" || taskText != "核对真实 transcript" {
		t.Fatalf("session metadata = %q %q %q %q %q %q %q, want first recorded values", started, ended, harness, model, cwd, title, taskText)
	}
}
