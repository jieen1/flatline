# Codex Adapter Field Matrix

P2 synthetic-fixture contract. Coverage describes what the adapter may claim, not what every source turn contains.

| Canonical field | Status | Mapping / evidence |
| --- | --- | --- |
| source | supported | Fixed adapter source `codex`. |
...
| asset invocation | supported | `turns[].asset_invocations[]`; maps to `invoked` + `asset_invoked`. |
...
| locator | supported | Source, qualified session, turn id/index, and stable raw reference. |
