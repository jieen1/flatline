...
## 5.1 · P18 落地记（2026-08-29 同日，方向修正后）

用户两次纠偏把方向从"经验层 CI"扩正为**会话语料是核心资产，价值=炼成经验并送到场**。
P18 四拍当日全部落地并经真实使用检验（`docs/qa/dogfood-2026-08-29.md` §23–27）：

| 拍 | 产出 | 真实读数 |
| --- | --- | --- |
| P18-1 解法挖掘 | `/friction/resolution`：签名终结点之后的动作，跨会话佐证 | 2,796 条矿脉；25/25 可挖、8/25 榜首 ≥2 佐证；先读后写签名的终结动作 read:Read 获 56 会话佐证——独立复证机制字典 |
| P18-2 到场·简报 | 签名简报"上次怎么过去的"区块 + 事件深链 | pkill exit 144：66/66 终结、git status ×3 佐证领衔 |
| P18-3 项目作业知识 | `/projects/{key}/knowledge`：命令级作业清单 | cognode 30 条，头部即"怎么看 issue/怎么开工/怎么跑 CI" |
| P18-4 开工简报 | `/projects/{key}/briefing`（markdown）+ `project-briefing` 技能 + 新工作树索引 | 闭环由 agent 本人当场走通；doctrine 文件被 112 会话读过直接进简报 |
