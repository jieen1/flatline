...
Flatline 是一台本地运行的 Agent 资产监护仪：它持续追踪每个 Skill、AGENTS.md、Rule、Hook 在真实工作会话中的生命体征，在资产静默失效、损坏或被绕行时告警，帮助用户诊断、修复或清理，并在修改后再次被观测到时确认修复是否生效。
...
2. **状态迁移才开口。**系统只在资产进入沉默/失效/被绕行、或完成修改后验证时主动通知；状态未变则永远沉默。这替代 v0.3 的"三种开口时刻"。
3. **删除是一等结局。**清理休眠资产与修复沉默资产同等重要；清理的收益（上下文占用下降、触发稀释减少）确定性可计算。
4. **验收以修改后验证为默认。**修改后的默认验证是"首次重新参与并被遵循"这个单事件，不是统计窗口。
...
| `no_opportunity` 没有相关任务记录 | 窗口内没有同形状 Session 记录 | 否 |
| `unobservable` 不可观测 | 数据源无法提供参与信号 | 否（如实展示） |
| `awaiting_resurrection` 修改后验证中 | 沉默/失效/被绕行资产被修改后，尚未观测到重新参与 | 验证完成时**是** |
...
stateDiagram-v2
    ...
    awaiting_resurrection --> healthy: 首次重新参与且被遵循（修改后验证通过，通知）
    awaiting_resurrection --> silent: 修改后 N 机会仍零参与（修改后验证未通过，通知）
...
- 修改后验证通过是正向通知，与问题告警同权重送达——修好东西并再次看到它参与的时刻需要被明确确认。
...
`awaiting_resurrection` 期间，首次观测到该资产重新参与且未违背 → 迁移至 healthy → 修改后验证通过通知（"修改后首次被重新调用：今天 14:32，Session #…"）。修改后 N 个机会仍零参与 → 修改后验证未通过通知，回到诊断。单事件判定，无最小样本。
...
正式改善轮次（目标指标 + 防回归 + 观察窗 + 统计验收）不进入产品：UI 不实现、MVP 代码不实现、对象模型不包含。设计完整存档于附录 B，仅当修改后验证被真实用户证明不足以支撑修改验证需求时重新评估。
...
必须完成：CC + Codex 适配器；资产快照与版本；机会/参与/基线追踪；**四个 detector：沉默判定、引用体检、调用后违背、休眠识别**；生命体征墙 + 诊断页 + 变化时间线；四种处置 + 修改后验证；批量清理流；Session 基础浏览（借鉴 AgentView 水准）；全链路观测等级与 locator 展示。
...
4. 从沉默告警到完成一次修改并收到修改后验证通过通知，全流程无需离开产品（外部编辑器除外）；
...
- **ADR-6 验收降级为修改后验证。**修改的默认验证是单事件（首次重新参与且未违背）；统计验收进入可选进阶模式。
...
**正式改善轮次（ImprovementCycle）**：围绕一次深思熟虑的资产修改建立目标指标、防回归指标、最小样本观察窗与统计验收。完整设计存档于《flatline-backlog-improvement-cycle.md》（状态机、confounded 规则、验收页规范；该历史文件未包含在本仓库）与 v0.3 §4-§5。MVP 不实现该能力，引用缺失不影响当前范围。搁置理由：统计验收的样本需求超出个人用户数据现实，环境变化快于样本累积，多数结论将是 inconclusive；修改后验证（单事件验证）以极低成本承担了同一需求。重启条件：真实用户明确反馈"修改后验证不足以让我信任修改效果"，且其 Session 量级足以在两周内积累可比样本。
