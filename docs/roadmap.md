...
## 方向（2026-08-29 调研收敛，详见 docs/flatline-direction-2026-08.md）

一条主线两翼：**经验层 CI**（规则/技能的 coverage → adherence 曲线 → staleness → 带回滚的退役，
全部从用户自己的历史纵向测量）；**舰队作业面**（闭环读数以舰队为单位）；
**事实层存档**（harness 30 天删转写，本库即唯一副本）。
明确不做：成本报表深化（红海）、通用可观测平台化（弃零接入护城河）。
落地拍 P17-1..5 见方向文档 §5。
