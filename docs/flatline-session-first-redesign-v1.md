...
### `GET /api/v1/friction/resolution`（P18-1 解法挖掘，2026-08-29）

一条签名"上次怎么过去的"：`total_sessions` / `ended_sessions`（终结 = 最后一次出现之后会话仍有
事件）、`actions[]`（终结点之后前 5 条动作，跨会话按规范化形式聚合，`sessions` 为佐证数；
裸文件动作只在失败工具本身是文件工具时参与排序——实测 `read:Read` 曾以填充噪音占据 12 条命令类
签名中 9 条的榜首；并列按更靠近终结点排序）、`sample`（携带该榜首动作的最近一次真实终结序列，
逐条带 event_id 可下钻）。判定句随响应携带："只陈述先后与对齐，不判定因果——随后发生的未必是解法"。
首版真实精度（25 条 ≥5 会话的签名）：25/25 可挖、8/25 榜首动作 ≥2 会话佐证；
